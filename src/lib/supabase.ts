...
const getSupabaseConfig = () => {
  // env var > hardcoded > localStorage (localStorage nunca pisa el default)
  const url = import.meta.env.VITE_SUPABASE_URL || SUPABASE_URL || localStorage.getItem('sb-url') || ''
  const key = import.meta.env.VITE_SUPABASE_ANON_KEY || SUPABASE_ANON_KEY || localStorage.getItem('sb-key') || ''
  return { url, key }
}
